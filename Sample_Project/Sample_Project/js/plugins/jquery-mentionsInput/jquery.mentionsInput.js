...
(function ($, _, undefined) {
    ...
    var utils = {
        ...
        //Decodes the character with _.unescape function (undersocre)
    ...
    var MentionsInput = function (settings) {
        ...
        function onInputBoxInput(e) {
            ...
            const isAndroid = /android|samsung/i.test(ua); //&& ua.indexOf("mobile");
            if (isAndroid && e.keyCode !== KEY.BACKSPACE) {//certain versions of android mobile browser don't trigger the keypress event.
                let char_code;
                const current_text = domInput.value;
                if (current_text.length < last_text.length) {
                    inputBuffer.pop();
                } else {
                    if (!e.which || !e.keyCode) char_code = current_text[current_text.length - 1].charCodeAt();
                    const typedValue = String.fromCharCode(char_code || e.which || e.keyCode); //Takes the string that represent this CharCode
                    if (current_text.includes('@')) inputBuffer.push(typedValue); //Push the value pressed into inputBuffer
                }
                last_text = domInput.value;
            }
        ...
        function onInputBoxKeyPress(e) {
            var ua = navigator.userAgent.toLowerCase();
            const isAndroid = /android|samsung/i.test(ua);
            if (!isAndroid) {
                if (e.keyCode !== KEY.BACKSPACE) { //If the key pressed is not the backspace
                    let char_code;
                    if (!e.which || !e.keyCode) {
                        char_code = domInput.value[domInput.value.length - 1].charCodeAt();
                    }
                    var typedValue = String.fromCharCode(char_code || e.which || e.keyCode); //Takes the string that represent this CharCode
                    if (domInput.value.includes('@') || typedValue === '@') inputBuffer.push(typedValue); //Push the value pressed into inputBuffer
                }
            }
        }
            ...
            _.each(results, function (item, index) {
                ...
                if (settings.showAvatars) {
                    ...
                    if (item.icon) {
                        ...
                        elmIcon = $(settings.templates.autocompleteListItemAvatar({ avatar: item.avatar, display: item.name }));
                    }
        ...
        return {
            ...
            //Resets the text area value and clears all mentions with default value
            resetValue: function (defaultValue) {
                resetInput(defaultValue);
            },
